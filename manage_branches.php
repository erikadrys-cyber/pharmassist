    ...
    if (!empty($branch_name) && !empty($branch_address)) {
        ...
        if ($stmt->execute()) {
            ...
            if ($old_name !== $branch_name) {
                $update_medicines = $conn->prepare("UPDATE medicine SET branch = ? WHERE branch = ?");
                $update_medicines->bind_param("ss", $branch_name, $old_name);
  ...
  <div class="home-content">
    ...
    <div class="branch-management-container">
      ...
      <div class="page-header">
        <h1><i class="bi bi-building"></i> Branch Management</h1>
      ...
      <div class="add-branch-form">
        <h3><i class="bi bi-plus-circle"></i> Add New Branch</h3>
        <form method="POST" action="manage_branches.php">
      ...
      <div class="branches-table-container">
        <h3><i class="bi bi-list-ul"></i> Existing Branches</h3>
  ...
  <script>
    ...
    function confirmDelete(id, name, medicineCount) {
      ...
      if (confirm(`Are you sure you want to delete "${name}"?\n\nThis action cannot be undone.`)) {
        window.location.href = `manage_branches.php?delete=${id}`;
      }
    }
